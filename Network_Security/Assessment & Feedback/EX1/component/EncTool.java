...
public class EncTool {
    ...
    private static void decryptAESCTR() {
        ...
        try
        {
            ...
            //get initial vector
            lengthiv = getContent.read(iv);
            System.out.println("Initial vector length is : " + lengthiv);
            System.out.println("Initial vector is : " + (byteArrayToHexString(iv)));

            FileReader reader = new FileReader("./iv_ctr.txt");
            BufferedReader bufferedReader = new BufferedReader(reader);

            while (true)
            {
                lengthcipher = getContent.read(cipher);
                String line;
                if (lengthcipher != -1)
                {
                    if ((line = bufferedReader.readLine()) != null)
                    {
                        iv = hexStringToByteArray(line);
                    }
                    System.out.println("-------------------------------------------");
                    System.out.println("IV is : " + byteArrayToHexString(iv));
                    System.out.println("Counter [" + counter + "]");
                    System.out.println("Length of cipher is : " + lengthcipher);
                    System.out.println("Hex of Cipher is : " + byteArrayToHexString(cipher));
                    System.out.println("-------------------------------------------");


                    byte[] plain = decryptCTR(cipher, iv, lengthcipher);
    ...
    private static byte[] decryptCTR(byte[] cipher, byte[] iv, int length)
    {
        ...
        try
        {
            ...
            plain = decAESCTRcipher.doFinal(cipher);
        } catch (Exception e)
        {
            System.out.println(e);
        }
